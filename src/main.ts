...
const fixedUrl = serverURL.replace(/\/?(\?|#|$)/, '/$1')

async function generateSvg(code) {
    const encoded = plantumlEncoder.encode(code);
    try {
        const res = await axios.get(fixedUrl + `svg/${encoded}`);
        return res.data;
    } catch(e) {
        // TODO
    }
...
async function generatePngBlob(code) {
    const encoded = plantumlEncoder.encode(code);
    try {
        const res = await axios.get(fixedUrl + `png/${encoded}`, { responseType: 'arraybuffer' });
        return res.data;
    } catch(e) {
        // TODO
    }
...
(async function main() {
    ...
    for (const plantumlCode of plantumlCodes) {
        ...
        const png = await generatePngBlob(plantumlCode.code);
        const pngContent = Buffer.from(png, 'binary').toString('base64');

        const blobRes2 = await octokit.git.createBlob({
            owner, repo,
            content: pngContent,
            encoding: 'base64',
        });

        const sha2 = await octokit.repos.getContents({
            owner, repo, ref, path: p2
        }).then(res => (<any>res.data).sha).catch(e => undefined);

        if (blobRes2.data.sha !== sha2) {
            tree = tree.concat({
                path: p2.toString(),
                mode: "100644",
                type: "blob",
                sha: blobRes2.data.sha
            })
        }
    }
